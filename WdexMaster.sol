...
contract WoonklySwapToken is ERC20("WoonklyDEX", "WDEX", 18, 0, 750000000), Ownable, Mintable {
...
contract WdexMaster is Ownable {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 accWdexPerShare;
    }

    WoonklySwapToken public wdex;
    uint256 public wdexPerBlock;
    ...
    constructor(
        WoonklySwapToken _wdex,
        uint256 _wdexPerBlock,
        uint256 _startBlock
    ) public {
        wdex = _wdex;
        wdexPerBlock = _wdexPerBlock;
    ...
    function pendingWdex(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        uint256 accWdexPerShare = pool.accWdexPerShare;
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 wdexReward = multiplier
                .mul(wdexPerBlock)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            accWdexPerShare = accWdexPerShare.add(
                wdexReward.mul(1e12).div(lpSupply)
            );
        }
        return
            user.amount.mul(accWdexPerShare).div(1e12).sub(user.rewardDebt).add(user.pendingRewards);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 wdexReward = multiplier
            ...
            .mul(pool.allocPoint)
            .div(totalAllocPoint);
        wdex.mint(address(this), wdexReward);
        pool.accWdexPerShare = pool.accWdexPerShare.add(
            wdexReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount, bool _withdrawRewards) public {
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                ...
                if (_withdrawRewards) {
                    safeWdexTransfer(msg.sender, user.pendingRewards);
        ...
        user.rewardDebt = user.amount.mul(pool.accWdexPerShare).div(1e12);
    ...
    function withdraw(uint256 _pid, uint256 _amount, bool _withdrawRewards) public {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accWdexPerShare).div(1e12).sub(user.rewardDebt);
        if (pending > 0) {
            user.pendingRewards = user.pendingRewards.add(pending);
    ...
    function safeWdexTransfer(address _to, uint256 _amount) internal {
        uint256 wdexBal = wdex.balanceOf(address(this));
        if (_amount > wdexBal) {
            wdex.transfer(_to, wdexBal);
        } else {
            wdex.transfer(_to, _amount);
        }
    }

    function setWdexPerBlock(uint256 _wdexPerBlock) public onlyOwner {
        require(_wdexPerBlock > 0, "!wdexPerBlock-0");
        wdexPerBlock = _wdexPerBlock;
    }
